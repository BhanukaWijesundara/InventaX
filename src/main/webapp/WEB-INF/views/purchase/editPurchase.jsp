...
<head>
  <meta charset="UTF-8">
  <title>Edit Purchase - InventaX</title>
        ...
        <div class="col-lg-8">
          <div class="card">
            <div class="card-header">
              <h5 class="mb-0"><i class="fas fa-edit me-2"></i> Purchase Details</h5>
            </div>
            <div class="card-body p-4">
              <form action="purchases" method="post">
                <input type="hidden" name="action" value="update">
                <input type="hidden" name="originalPurchaseId" value="${purchase.purchaseId}">

                <div class="mb-3">
                  <label for="purchaseId" class="form-label">Purchase ID</label>
                  <input type="text" class="form-control" id="purchaseId" name="purchaseId" value="${purchase.purchaseId}" readonly>
                  <div class="form-text">Purchase ID cannot be changed.</div>
                </div>
                ...
                <div class="mb-3">
                  ...
                  <select class="form-select" id="itemId" name="itemId" required>
                    ...
                    <c:forEach items="${items}" var="item">
                      <option value="${item.itemId}" ${item.itemId eq purchase.itemId ? 'selected' : ''}>
                ...
                <div class="mb-3">
                  ...
                  <select class="form-select" id="supplierId" name="supplierId" required>
                    ...
                    <c:forEach items="${suppliers}" var="supplier">
                      <option value="${supplier.supplierId}" ${supplier.supplierId eq purchase.supplierId ? 'selected' : ''}>
                ...
                <div class="mb-3">
                  ...
                  <input type="date" class="form-control" id="purchaseDate" name="purchaseDate" value="${purchase.date}" required>
                </div>

                <%-- Removed other fields if not in PurchaseOrder model --%>
                <%-- If they exist, uncomment and ensure names match the model fields passed from servlet --%>
                <%--
                <div class="mb-3">
                    <label for="totalAmount" class="form-label">Total Amount</label>
                    <div class="input-group">
                        <span class="input-group-text">$</span>
                        <input type="number" class="form-control" id="totalAmount" name="totalAmount" step="0.01" min="0" value="${purchase.totalAmount}" required>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="paymentStatus" name="paymentStatus" required>
                        <option value="pending" ${purchase.paymentStatus eq 'pending' ? 'selected' : ''}>Pending</option>
                        <option value="paid" ${purchase.paymentStatus eq 'paid' ? 'selected' : ''}>Paid</option>
                        <option value="cancelled" ${purchase.paymentStatus eq 'cancelled' ? 'selected' : ''}>Cancelled</option>
                    </select>
                </div>
